...
rm -f  /tmp/kill_all_process.*   1>/dev/null 2>&1

isql -U$SAUSER -P$SAPWD -S$ASE_NAME  <<  EOF > /tmp/kill_all_process.out
select 'PROC',p.spid, p.hostname, p.suid from sysprocesses p, sysdatabases d where d.dbid=p.dbid and p.suid > 0 and d.name <> 'master'
...
OKSELECTPROCESS=`grep okselectprocess /tmp/kill_all_process.out | wc -l`
...
cat /tmp/kill_all_process.out | awk '/PROC/ { print "kill ",$2;
                                          print "if @@error != 0 select \047 errorekill \047";
                                          print "go"; }' > /tmp/kill_all_process.sql
echo exit >> /tmp/kill_all_process.sql

# Eseguo lo script sql che effettua i kill

isql -U$SAUSER -P$SAPWD -S$ASE_NAME -i /tmp/kill_all_process.sql > /tmp/kill_all_process.esitokill

# Se trovo la stringa errorekill nell'output vuol dire che c'e' stato un errore

ERRKILL=`grep errorekill /tmp/kill_all_process.esitokill | wc -l`
#echo $ERRKILL
