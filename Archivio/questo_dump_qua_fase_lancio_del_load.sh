...
echo sono in questo_dump_qua_fase_lancio_del_load.sh

rm -f $DUMP_DIR/dumpmx_1shot_singolodb_questo_dump_qua_mettilo_laa

# Attendo DURATA_MINUTI minuti che appaia il file dumpmx_1shot_singolodb_questo_dump_qua_mettilo_laa
DURATA_MINUTI=60
countdown_minuti=$DURATA_MINUTI
echo countdown_minuti=$countdown_minuti
while [ $countdown_minuti -gt 0 -a ! -s $DUMP_DIR/dumpmx_1shot_singolodb_questo_dump_qua_mettilo_laa ]
do
   #echo MInutiSecondi rimasti $countdown_secondi
   #echo Prima dello sleep 
   sleep 60
   countdown_minuti=`expr $countdown_minuti \- 1`
   #echo Termine sleep $countdown_minuti
   echo $(/usr/bin/date +%Y%m%d-%H%M) countdown_minuti $countdown_minuti  >> /tmp/questo_dump_qua_fase_lancio_del_load_countdown
done

if [ ! -s $DUMP_DIR/dumpmx_1shot_singolodb_questo_dump_qua_mettilo_laa ] ; then
   echo Non arriva il file dumpmx_1shot_singolodb_questo_dump_qua_mettilo_laa
   segnalazione_errore
   exit 1;
else
   echo Cancello il file dumpmx_1shot_singolodb_questo_dump_qua_mettilo_laa
