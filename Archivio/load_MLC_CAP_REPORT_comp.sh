...
NOMEDB=MLC_CAP_REPORT
NOMEDUMP=MLC_CAP_PROD_TO_LOAD-NOZIP.cmp.dmp
...
if [ ! -f /sybased1/dump/MLC_CAP_PROD_TO_LOAD-NOZIP.cmp.dmp ] ; then
   echo "Dump non trovato per script load_MLC_CAP_REPORT_comp.sh" 
   echo "Il dump non va cancellato perche' deve rimanere come"
   echo "segnaposto"
   echo "    "
   REPNAGIOS=/nagios_reps/Fallimento_load_MLC_CAP_REPORT_$DATA
   echo "Fallimento load_MLC_CAP_REPORT_comp.sh" > $REPNAGIOS
   echo "Caso dump non trovato" >> $REPNAGIOS
   echo "Rifare trasferimento da MLC_CAP_PROD a MLC_CAP_REPORT con loadmx_preservando_sysusers.sh" >> $REPNAGIOS
...
# L'ESITO NON VA CANCELLATO QUI MA NEL SUCCESSIVO load_CAP_REPORT rm -f $UTILITY_DIR/out/esito_ultimo_dumpmx_alldb_su_prod 2>/dev/null
...
#load database $NOMEDB from "compress::/sybased1/dump/MLC_CAP_PROD_TO_LOAD-NOZIP.cmp.dmp"
...
if [ $LOAD_IS_COMPLETE -eq 0 ] ; then
   echo "ERRORE NELLA LOAD DI MLC_CAP_REPORT!"
   echo "Load da rifare!"
   echo "    "
   REPNAGIOS=/nagios_reps/Fallimento_load_MLC_CAP_REPORT_$DATA
   echo "Fallimento load_MLC_CAP_REPORT_comp.sh" > $REPNAGIOS
...
#rm /sybased1/dump/MLC_CAP_PROD_TO_LOAD-NOZIP.cmp.dmp
