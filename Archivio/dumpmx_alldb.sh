...
if [ $NUMERO_REPORT_PRESENTI -ge 7 ] ; then
...
do
       sleep 10
       sh $UTILITY_DIR/dumpmx_singolodb.sh $i $DATA
       sleep 20
       #echo $i
...
if [ $NUMERO_RIGHE_REPORT -ne $NUMERODB -o $ERRORI_NEL_REPORT -gt 0 ] ; then
   ...
   #/usr/sbin/sendmail -f dumpmxerr@kyneste.com esantanche@tim.it < /tmp/dumpmx.mail
   /usr/sbin/sendmail -f dumpmxerr@kyneste.com dba@kyneste.com < /tmp/dumpmx.mail
   echo "Errore nel dump "$DATA" dell'ASE "$ASE_NAME > /tmp/dumpmx.nagiosrep
   echo "Provvedere immediatamente a rifare il dump lanciando lo script dumpmx_alldb.sh" >> /tmp/dumpmx.nagiosrep
   echo "presente nella directory /sybase/utility loggandosi con l'utenza sybase" >> /tmp/dumpmx.nagiosrep
   cp /tmp/dumpmx.nagiosrep /nagios_reps/Errore_dump_$DATA
   rm /tmp/dumpmx.nagiosrep
   rm -f /tmp/dumpmx.mail
   CODICE_RITORNO=1
else
   echo Dump ok $DATA > $UTILITY_DIR/out/esito_ultimo_dumpmx_alldb
   #DURATA_MIN=`expr \( $FINE_ORE \* 60 \) \+ $FINE_MIN  \- \( $INIZIO_ORE \* 60 \) \- $INIZIO_MIN `
   #if [ $DURATA_MIN -lt 0 ] ; then
   #   DURATA_MIN=`expr $DURATA_MIN \+ 1440`   
   #fi
   #echo "$(/usr/bin/date +%Y%m%d-%H%M) Inizio ore "$$INIZIO_ORE $(/usr/bin/date +%H%M)
   #echo $DATA $INIZIO_ORE $INIZIO_MIN $FINE_ORE $FINE_MIN | nawk '{ inizio_ore=$2;
   #                                                                 inizio_min=$3;
   #                                                                 fine_ore=$4;
   #                                                                 fine_min=$5;
   ...
   #                                                                 if (diff < 0) { diff += 1440; };
   #
   #                                                                 printf "%-8s   Inizio ore %02d:%02d",$1,inizio_ore,inizio_min;
   #                                                                 printf "   Fine ore %02d:%02d",fine_ore,fine_min;
   #                                                                 printf "   Durata ore %02d:%02d\n",int(diff/60),diff % 60; 
   #                                                               }' >> $UTILITY_DIR/out/log_tempo_impiegato_dump 
   echo "Subject: [DUMPMX] Dump ok "$DATA" "$ASE_NAME  >  /tmp/dumpmx.mail
   echo "To: dba@kyneste.com"  >> /tmp/dumpmx.mail
   echo "Dump "$DATA" eseguito regolarmente sull'ASE "$ASE_NAME >> /tmp/dumpmx.mail
   echo "sulla macchina "`hostname` >> /tmp/dumpmx.mail
   echo "   " >> /tmp/dumpmx.mail
   #echo "Cronologia tempi di esecuzione ultimi 10 giorni" >> /tmp/dumpmx.mail
   #echo "================================" >> /tmp/dumpmx.mail
   #tail -10 $UTILITY_DIR/out/log_tempo_impiegato_dump >> /tmp/dumpmx.mail
   /usr/sbin/sendmail -f dumpmxok@kyneste.com dba@kyneste.com < /tmp/dumpmx.mail
   rm -f /tmp/dumpmx.mail
   CODICE_RITORNO=0
fi

ls -1 $PATH_DUMP_DIR > $UTILITY_DIR/out/lista_dir_bkp_notte

sh $UTILITY_DIR/centro_log_operazioni.sh DUMP ALL $CODICE_RITORNO $INIZIO_ORE $INIZIO_MIN $FINE_ORE $FINE_MIN
