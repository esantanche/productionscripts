...
echo script in lavorazione non uusare
...
if [ $1a = a ] ; then
   echo "Usage: dare come parametro il nome del db da clonare"
   exit 0
fi
...
isql -U$SAUSER -P$SAPWD -S$ASE_NAME -w400  <<  EOF > /tmp/dbclonemx_singolodb
declare @sumdata int, @sumlog int
select @sumdata = 0
select @sumlog = 0
select @sumdata = sum(size) from sysusages where dbid=(select dbid from sysdatabases where name='$NOMEDB') and segmap=3
select @sumlog  = sum(size) from sysusages where dbid=(select dbid from sysdatabases where name='$NOMEDB') and segmap=4
if @@error != 0 select 'ERRORE'
select 'DATALOG',@sumdata,@sumlog
if @@error != 0 select 'ERRORE'
go
exit
EOF

SIZE_DATA_MB=`cat /tmp/dbclonemx_singolodb | awk '/DATALOG/ { print $2*4096/1048576; }'`
SIZE_LOG_MB=`cat /tmp/dbclonemx_singolodb | awk '/DATALOG/ { print $3*4096/1048576; }'`

echo Nome db origine $NOMEDB
echo Size parte dati in mb $SIZE_DATA_MB
echo Size parte log in mb  $SIZE_LOG_MB
echo Nome db clone $NOMEDB@BCP

#disk init name = "<nome device>", physname = "/sybased2/databases/<nome file>", size = "<size in mb>M"
#create database <nome db> on <nome device> = "<size in mb del device>M" log on <nome device> = "<size in mb del device>M" 
...
select "DEVICE_DA_AMPLIARE",max(name) from sysdevices where name like "${NOMEDB}#D%"
go
exit
EOF`
...
if [ $DEVICE_DA_AMPLIARE = 'NULL' ] ; then
   echo "Il db " $NOMEDB " non ha il device con suffisso #Dnn"
   echo "ovvero di nome ${NOMEDB}#Dnn."
