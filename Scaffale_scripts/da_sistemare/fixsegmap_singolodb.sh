...
echo Fix Segmap output db $NOMEDB > /tmp/fixsegmap_output
...
go
select 'segmap a 4 per i device log ovvero il cui nome ha formato nomedb#Lnn'
go
update sysusages set segmap = (4 | (segmap & (~ 7))) where vstart in (select u.vstart 
           from master.dbo.sysusages u, master.dbo.sysdevices d where u.vstart between d.low and d.high
           and u.dbid = db_id('$NOMEDB') and d.name like '%#L%')
go
select 'segmap a 3 per i device dati ovvero il cui nome ha formato nomedb#Dnn'
go
update sysusages set segmap = (3 | (segmap & (~ 7))) where vstart in (select u.vstart    
           from master.dbo.sysusages u, master.dbo.sysdevices d where u.vstart between d.low and d.high
           and u.dbid = db_id('$NOMEDB') and d.name like '%#D%')
go
