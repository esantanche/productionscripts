...
rm -f $SUNDAYDIR/bcp-lista-tabelle-$NOMEDB

#NUMTAB=0 # ????????? TBD da togliere
ERRORI_PRESENTI=0
for i in `isql -U$SAUSER -P$SAPWD -S$ASE_NAME  <<  EOF | egrep -v 'name|----------|sysgams|syslogs' 
set nocount on
go
use $NOMEDB
go
select  '$NOMEDB.' + u.name +  '.'  + o.name from sysobjects o, sysusers u where u.uid=o.uid and o.type in ('U') order by u.name,o.name
go
quit
EOF`
do
   echo $(/usr/bin/date +%Y%m%d-%H%M%S) BCP di $i db $NOMEDB >>$UTILITY_DIR/out/esito_bcp_output_ultimo_bcp 
   echo $i >>$SUNDAYDIR/bcp-lista-tabelle-$NOMEDB
   bcp $i out $SUNDAYDIR/bcp-$i.native.bcp -e $SUNDAYDIR/bcp-$i.err -n -b 10 -T 1320000 -P $SAPWD -S $ASE_NAME -U $SAUSER 2>&1 >>$UTILITY_DIR/out/esito_bcp_output_ultimo_bcp
   ERROREBCP=$?
...
if [ $ERRORI_PRESENTI -eq 1 ] ; then
   ...
   find $SUNDAYDIR | grep .bcp.gz > $SUNDAYDIR/bcp-lista-files-$NOMEDB
   tar -cvf $SUNDAYDIR/bcp-$NOMEDB.tar -L $SUNDAYDIR/bcp-lista-files-$NOMEDB 1>/dev/null
   # poi devo cancellare tutti i bcp
   for j in `cat $SUNDAYDIR/bcp-lista-files-$NOMEDB`
   do
      rm -f $j
   done 
fi
