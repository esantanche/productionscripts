...
if [ ! -f $NOMESTRIPE3 ] ; then
   ...
   echo quindi non esco con errore
fi
...
echo "Seconda stripe: "$NOMESTRIPE2
echo "Terza stripe:   "$NOMESTRIPE3
echo " "
echo "Si ricorda che:"
echo " "
echo "1) Il db" $NOMEDB "deve essere di dimensione uguale o maggiore del db di partenza"
echo " "
echo "2) Questo script gestisce solo dump *COMPRESSI* e *DIVISI IN DUE O TRE STRIPE*"
echo " "
echo "3) I file di dump, dopo il caricamento, *NON* vengono eliminati"
echo " "
echo "4) Questo script lancia lo script loadmx_in_crontab_striped.sh che quando finisce"
echo "   manda una mail"
echo "  "
echo " "
...
echo "Lancio lo script loadmx_in_crontab_striped.sh" $NOMEDB $NOMESTRIPE1
echo "Lo lancio con nohup e in background"
rm -f nohup.out
if [ $USER != sybase ] ; then
   su - sybase -c "nohup sh $UTILITY_DIR/loadmx_in_crontab_striped.sh $NOMEDB $NOMESTRIPE1 &"
else 
   nohup sh $UTILITY_DIR/loadmx_in_crontab_striped.sh $NOMEDB $NOMESTRIPE1 &
fi
