...
do
       ...
       /sybase/sybcent32/ddlgen -U$SAUSER -P$SAPWD -S`hostname`:4100 -D$i -T${TIPO} -O$SUNDAYDIR/ddlmx_$i_${TIPO}.sql -E$SUNDAYDIR/ddlmx_$i_${TIPO}.err 
       ERROREDDL=$?
       if [ $ERROREDDL -gt 0 ] ; then
          echo ERRORE NEL DDL DI $i
          exit 1
       fi
       if [ -s $SUNDAYDIR/ddlmx_$i_$TIPO.err ] ; then
          echo ERRORE IN FILE ERRORI DI DDL $i
          exit 1
       else
          rm -f $SUNDAYDIR/ddlmx_$i_$TIPO.err
       fi
       sleep 20
done
