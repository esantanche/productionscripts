...
#$SUNDAYDIR/lista_files_$NOMEDB

#echo $i >>$SUNDAYDIR/bcp-lista-tabelle
#find $SUNDAYDIR | grep .bcp.gz > $SUNDAYDIR/bcp-lista-files-$NOMEDB

rm -f /tmp/bcpin-delete-script

echo "use "$NOMEDB > /tmp/bcpin-delete-script
echo "go" >> /tmp/bcpin-delete-script

for i in `cat $SUNDAYDIR/bcp-lista-tabelle`
do
   echo "delete from "$i >> /tmp/bcpin-delete-script
   echo "go " >> /tmp/bcpin-delete-script
done

#cat /tmp/bcpin-delete-script

#exit 0

isql -U$SAUSER -P$SAPWD -S$ASE_NAME -i /tmp/bcpin-delete-script > $SUNDAYDIR/bcpin-fase-delete

exit 0

rm -f $SUNDAYDIR/bcpin-fase-bcp

for i in `cat $SUNDAYDIR/bcp-lista-tabelle`
do
   echo bcpin $i
   bcp $i in $SUNDAYDIR/bcp-$i.bcp -e $SUNDAYDIR/bcp-$i.err -c -b 10 -T 1320000 -P $SAPWD -S $ASE_NAME -U $SAUSER 2>&1 >> $SUNDAYDIR/bcpin-fase-bcp 
done
